...
class Discovery extends Discover {
  constructor(advertisement, options) {
    ...
    this.on('added', obj => {
      if (!options.monitor && obj.advertisement.key != this.advertisement.key)
        return

      options.log &&
        options.statusLogsEnabled &&
        options.helloLogsEnabled &&
  ...
  statusLogger(obj, status) {
    ...
    if (status) {
      ...
      logs.push(
        ...
        '>',
        obj.advertisement.type.magenta + statusLog
      )
    } else {
      logs.push()
    }
