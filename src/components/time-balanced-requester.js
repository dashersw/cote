    ...
    setInterval(() => {
      ...
      for (const index in this.responders) {
        for (const id in this.responders[index]) {
          if (
            now - this.responders[index][id].sent >
            this.CALCULATION_TIMEOUT
          ) {
            if (this.timers[id]) {
      ...
      if (sock.uuid) {
        ...
        for (const id in responder) {
          ...
          if (typeof this.callbacks[id] == 'function') {
            ...
            this.callbacks[id] &&
              this.callbacks[id].reject(new Error('lost connection'))
          }
  ...
  send(...args) {
    ...
    const cb = (...args) => {
      ...
      if (hasCallback) {
        ...
        if (originalPromise.state == 'rejected')
          this.callbacks[id].reject(args[0])
