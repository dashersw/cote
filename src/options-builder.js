...
module.exports = options => {
  ...
  if (
    !process.env.COTE_BROADCAST_ADDRESS &&
    process.env.DOCKERCLOUD_IP_ADDRESS
  ) {
    environmentSettings.broadcast = '10.7.255.255'
  }

  const keys = Object.keys(process.env).filter(
    k => k.slice(0, 15) == 'COTE_DISCOVERY_'
  )
  ...
  keys.forEach(k => {
    ...
    const pluginObj = (environmentSettings[pluginName] =
      environmentSettings[pluginName] || {})

    keyArray.forEach(k => {
      pluginObj[k] =
        process.env[
          `COTE_DISCOVERY_${pluginName.toUpperCase()}_${k.toUpperCase()}`
        ]
    })
