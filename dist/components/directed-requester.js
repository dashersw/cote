...
module.exports = class DirectedRequester extends Requester {
  ...
  socketSend(...args) {
    ...
    const data = args[0]; // Requires selector parameter
    ...
    }).filter(sock => sock); // Load balance between sockets

    const sock = possibleSocks[this.n++ % possibleSocks.length]; // Enqueue if the correct sock did not connect yet/does not exist

    if (!sock) return this.sock.enqueue(args); // Save callback
    ...
    args.push(fn.id); // Remove identifier from message

    delete args[0][IDENTIFIER]; // Send over sock

    sock.write(requesterSock.pack(args));
  }

};
//# sourceMappingURL=directed-requester.js.map