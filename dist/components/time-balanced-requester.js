...
module.exports = class TimeBalancedRequester extends Requester {
  constructor(advertisement, discoveryOptions) {
    ...
    this.SAMPLE_INTERVAL = 100; // neglect requests from calculation this took longer then the given timeout
      ...
      for (let index in this.responders) {
        for (let id in this.responders[index]) {
          if (now - this.responders[index][id].sent > this.CALCULATION_TIMEOUT) {
            if (this.timers[id]) clearInterval(this.timers[id]); // if the response is lost, don't leave garbage

            delete this.responders[index][id];
          }
        }
      }
    }, this.SAMPLE_INTERVAL); // clear all gathered data when a socket disconnects
  ...
  send(...args) {
    ...
    const cb = (...args) => {
      ...
      delete this.timers[id];
      const stats = {
        averageTime: lastRequests && lastRequests.average || 0
      };

      if (hasCallback) {
        this.callbacks[id](...args, stats);
      } else {
        if (originalPromise.state == 'rejected') this.callbacks[id].reject(args[0]);else this.callbacks[id].resolve(args[0]);
      }
    ...
    if (hasCallback) return super.send(...args, cb);else {
