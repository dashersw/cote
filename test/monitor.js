...
const { Requester, Responder, Monitor } = require('../')({
  environment,
  statusInterval: 100,
})
...
test.cb('Monitor throws unknown error', t => {
  ...
  const monitor = new Monitor({
    name: `${t.title}: error throwing monitor`,
    key,
  })
  monitor.sock.sock.on('bind', () =>
    monitor.sock.sock.server.emit('error', new Error('unknown error'))
  )
})
